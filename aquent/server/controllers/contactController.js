...
contactController.createContact = (req, res, next) => {
  ...
  const statement =
    'INSERT INTO contacts (first_name, last_name, email, street, state, city, zip_code) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *';
...
contactController.updateContactCompany = (req, res, next) => {
  const companyName = req.params.company_id ? 'companies.name' : '';
  const statement = `UPDATE contacts SET company_id = ${req.params.company_id}, company_name = ${companyName} FROM companies ON companies._id = req.params.company_id WHERE _id = ${req.params.contact_id} RETURNING *`;
  db.query(statement, (err, result) => {
...
contactController.removeContact = (req, res, next) => {
  const statement = `DELETE FROM contacts WHERE _id = ${req.params.contact_id}`;
  db.query(statement, (err, result) => {
